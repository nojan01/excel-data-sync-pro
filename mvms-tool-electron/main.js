...
function createWindow() {
    ...
    // Fenster-Referenz aufr�umen wenn geschlossen
...
ipcMain.handle('excel:readSheet', async (event, filePath, sheetName) => {
    try {
        ...
        // Hilfsfunktion: Pr�fen ob ein Zellwert ein Datum ist
        function isExcelDate(cell, value) {
            if (typeof value !== 'number') return false;
            
            // Pr�fe das Zahlenformat der Zelle
...
// Datei kopieren (fuer "Neuer Monat") - BIN�RE KOPIE erh�lt 100% Formatierung!
ipcMain.handle('excel:copyFile', async (event, { sourcePath, targetPath, sheetName, keepHeader }) => {
    try {
        // Wenn keepHeader false ist oder nicht gesetzt, einfach bin�r kopieren
        // Das erh�lt 100% der Formatierung!
        if (!keepHeader) {
            fs.copyFileSync(sourcePath, targetPath);
            return { success: true, message: `Datei kopiert: ${targetPath}` };
        }
        
        // Wenn keepHeader true ist, m�ssen wir die Daten l�schen
        // Aber zuerst: Bin�r kopieren, dann nur die Werte l�schen
        fs.copyFileSync(sourcePath, targetPath);
...
ipcMain.handle('config:loadFromAppDir', async (event) => {
    ...
    try {
        ...
        // Suchpfade in Priorit�tsreihenfolge
        const possiblePaths = [];
        
        // 1. Portable EXE: Neben der EXE (h�chste Priorit�t f�r portable Version)
        if (portableDir) {
